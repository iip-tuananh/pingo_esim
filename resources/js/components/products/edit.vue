			...
			<div class="col-md-8 grid-margin stretch-card">
				<div class="card">
					<div class="card-body">
						...
						<div class="form-group">
							<label>Ảnh title</label>
							<ImageMulti v-model="objData.images" :title="'san-pham'" />
						</div>
						<!-- <div class="form-group">
							<label>
								Thêm biến thể (Bạn có thể thêm biến thể nếu title này có nhiều lựa
								chọn như kích cỡ hoặc màu sắc. )
							</label>
						...
						<div
							class="variant"
							v-if="objData.status_variant == true"
						...
						>
								...
								<div>
									<vs-table :data="VariantSku">
										...
										<template slot-scope="{ data }">
											<vs-tr
												:key="indextr"
												v-for="(tr, indextr) in VariantSku"
											>
												<vs-td>
							...
							<div
								class="variant"
								v-if="objData.status_variant == true && VariantSku.length == 0"
							>
			...
			<div class="col-md-4 grid-margin stretch-card">
				<div class="card">
					<div class="card-body">
						...
						<div class="form-group">
							<label>Thẻ tags cho title</label>
							<vs-select
								multiple
								class="selectExample"
...
export default {
	...
	methods: {
		...
		saveProducts() {
			...
			if (this.objData.category == 0) this.errors.push('Chọn danh mục title');
			if (this.errors.length > 0) {
				...
				this.saveProduct(this.objData)
					.then(response => {
						...
						this.$notify.success('Sửa title thành công');
						// this.$route.push({name:'listProduct'});
					})
					.catch(error => {
						this.loadings(false);
						this.$notify.error('Sửa title thất bại');
					});
			}
		},
		listVariantSkuValue() {
